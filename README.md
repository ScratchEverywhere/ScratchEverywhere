# Scratch-3DS
A work in progress runtime made in C++ aimed to bring Scratch 3 projects over to the Nintendo 3DS, GameCube, Wii, and Wii U.
...
- **[GameCube, Wii, Wii U]** The first controller connected will be the only one that will work.
...
- Bug fixing and parity with Scratch
...
Got it! Here's your modified text with the dependencies reordered alphabetically, while preserving the structure and tone of the original as closely as possible:

---

#### Manual

If you are compiling with cloud variables, you will need to have [Mist++](https://github.grady.link/mistpp), a modified version of libcurl (instructions in mistpp-packages repo), and devkitpro's SDKs installed.

- For the 3DS you will need the DevkitARM toolchain and libctru. You will also need a 3DS compiled version of SDL2 and SDL2_mixer. See the [nightly build commands](https://github.com/NateXS/Scratch-3DS/blob/main/.github/workflows/nightly-3ds.yml) for a reference on how to compile SDL2 3DS for yourself

- For the GameCube you need all SDL2-gamecube libraries, the devkitPPC toolchain, [libromfs-ogc](https://github.com/NateXS/libromfs-ogc), and libogc

- For the Wii you need all SDL2-wii libraries, the devkitPPC toolchain, [libromfs-ogc](https://github.com/NateXS/libromfs-ogc), and libogc

- For the Wii U you will need all SDL2-wiiu libraries, the devkitPPC toolchain, [libromfs-wiiu](https://github.com/yawut/libromfs-wiiu), and WUT
...
- For the Wii U you need to run `make PLATFORM=wiiu all package` and then copy the `build/wiiu/scratch-wiiu` folder into the `sdcard:/wiiu/apps` folder on your Wii U SD card
