...
#define SCRATCH_BLOCK(category, id)                                                                                           \
    BlockResult block_category##_##id##_(Block &block, Sprite *sprite, bool *withoutScreenRefresh, bool fromRepeat);          \
    static uint8_t block_category##_##id##_reg_ = (BlockExecutor::handlers[#category "_" #id] = block_category##_##id##_, 0); \
    BlockResult block_category##_##id##_(Block &block, Sprite *sprite, bool *withoutScreenRefresh, bool fromRepeat)
#define SCRATCH_BLOCK_NOP(category, id) \
    static uint8_t block_category##_##id##_reg_ = (BlockExecutor::handlers[#category "_" #id] = nopBlock, 0);
#define SCRATCH_REPORTER_BLOCK(category, id)                                                                                       \
    Value block_category##_##id##_(Block &block, Sprite *sprite);                                                                  \
    static uint8_t block_category##_##id##_reg_ = (BlockExecutor::valueHandlers[#category "_" #id] = block_category##_##id##_, 0); \
    Value block_category##_##id##_(Block &block, Sprite *sprite)
