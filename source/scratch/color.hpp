...
inline ColorRGB HSB2RGB(const Color &color) {
    ...
    const float X = C * (1 - fabs(fmod(color.hue / 60.0, 2) - 1));
...
inline Color RGB2HSB(const ColorRGB &color) {
    ...
    if (diff == 0) goto end;
    if (cmax == r) return {static_cast<float>(fmod(60 * ((g - b) / diff) + 360, 360)), s, cmax * 100};
    if (cmax == g) return {static_cast<float>(fmod(60 * ((b - r) / diff) + 120, 360)), s, cmax * 100};
    if (cmax == b) return {static_cast<float>(fmod(60 * ((r - g) / diff) + 240, 360)), s, cmax * 100};
