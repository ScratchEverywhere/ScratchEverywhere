...
// element, per frame - very expensive on weak/embedded SDL3 targets. Instead,
// cache the rendered texture per (font, size, color, content) and only
// regenerate it when that combination actually changes. Entries unused for a
// while are evicted.
namespace {
struct CachedText {
...
static void SDL_RenderCornerBorder(SDL_Renderer *renderer, Clay_BoundingBox *boundingBox, Clay_BorderRenderData *config, int cornerIndex, Clay_Color _color) {
    ...
    for (int i = 0; i < numOuterTriangles; i++) {
        ...
        vertices[vertexCount++] = (innerRadius > 0) ? (SDL_Vertex){{centerX + SDL_cosf(angle2) * (innerRadius), centerY + SDL_sinf(angle2) * (innerRadius)}, color, {0, 0}} : (SDL_Vertex){{centerX, centerY}, color, {0, 0}};
        indices[indexCount++] = vertexCount - 1;
...
void Clay_SDL_Render(SDL_Renderer *renderer, Clay_RenderCommandArray renderCommands, SDL_Font *fonts) {
    ...
    for (uint32_t i = 0; i < renderCommands.length; i++) {
        ...
        case CLAY_RENDER_COMMAND_TYPE_TEXT: {
            ...
            SDL_RenderTexture(renderer, it->second.texture, NULL, &destination);
            break;
        }
