...
void TextObjectSDL3::uploadAtlas(FontGeneration &gen) {
    if (!renderer) return;
...
void TextObjectSDL3::render(int xPos, int yPos) {
    ...
    SDL_FColor col;
    col.r = ((color >> 24) & 0xFF) / 255.0f;
    col.g = ((color >> 16) & 0xFF) / 255.0f;
    col.b = ((color >> 8) & 0xFF) / 255.0f;
    col.a = (color & 0xFF) / 255.0f;
