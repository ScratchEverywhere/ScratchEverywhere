...
double SpeechManagerGLCore::getCurrentTime() {
    auto now = std::chrono::system_clock::now();
    auto duration = now.time_since_epoch();
    return std::chrono::duration_cast<std::chrono::milliseconds>(duration).count() / 1000.0;
}

void SpeechManagerGLCore::createSpeechObject(Sprite *sprite, const std::string &message) {
    speechObjects[sprite] = std::make_unique<SpeechTextObjectGLCore>(message, 200);
}

void SpeechManagerGLCore::render(int offsetX, int offsetY) {
    ...
    for (auto &[sprite, obj] : speechObjects) {
        if (obj && sprite->visible) {
            ...
            SpeechTextObjectGLCore *speechObj = static_cast<SpeechTextObjectGLCore *>(obj.get());
...
void SpeechManagerGLCore::renderSpeechIndicator(Sprite *sprite, int spriteCenterX, int spriteCenterY, int spriteTop, int spriteLeft, int spriteRight, int bubbleX, int bubbleY, int bubbleWidth, int bubbleHeight, double scale) {
