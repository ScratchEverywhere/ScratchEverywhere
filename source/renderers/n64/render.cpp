...
#include <log.hpp>
#include <windowing/n64/window.hpp>
...
surface_t *zbuf;
bool Render::Init() {
    globalWindow = new WindowN64();
    globalWindow->init(320, 240, "Scratch Everywhere!");

    /*if (!globalWindow->init(320, 240, "Scratch Everywhere!")) {
        delete globalWindow;
...
int Render::getWidth() {
    if (globalWindow) return 320;
    return 320;
}

int Render::getHeight() {
    if (globalWindow) return 240;
    return 240;
}
