...
void ProjectMenu::render() {
    ...
    if (hasProjects) {
        if (projectControl->selectedObject->isPressed({"a"})) {
            ...
            if (projectControl->selectedObject->buttonTexture->image->getID().find("projectBoxFast") != std::string::npos) {
                // Unpacked sb3
                Unzip::filePath = OS::getScratchFolderLocation() + projectControl->selectedObject->text->getText();
                MenuManager::loadProject();
                return;
            } else if (projectControl->selectedObject->buttonTexture->image->getID().find("projectBox") != std::string::npos) {
                // normal sb3
