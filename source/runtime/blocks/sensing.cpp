...
SCRATCH_BLOCK(sensing, of) {
    ...
    for (Sprite *currentSprite : Scratch::sprites) {
        if ((currentSprite->name == object.asString() || (object.asString() == "_stage_" && currentSprite->isStage)) && !currentSprite->isClone) {
...
SCRATCH_BLOCK(sensing, distanceto) {
    ...
    for (Sprite *currentSprite : Scratch::sprites) {
        if (currentSprite->name != distanceTo.asString() || currentSprite->isClone) continue;
...
#endif
    if (Scratch::useCustomUsername) *outValue = Value(Scratch::customUsername);
