...
nonstd::expected<extensions::Extension, std::string> extensions::parseMetadate(std::istream &data) {
    ...
    if (!data.read(magicBuffer.data(), magicString.length())) return nonstd::unexpected("Could not read " + std::to_string(magicString.length()) + " bytes for magic string.");
    ...
    if (!data.read(permissionBytes, 2)) return nonstd::unexpected("Could not read 2 bytes for permissions. Fille too short or I/O error.");
    ...
    if (!data.read(platformBytes, 2)) return nonstd::unexpected("Could not read 2 bytes for supported platforms. Fille too short or I/O error.");
    ...
    for (int i = 0; i < numSettings; i++) {
        ...
        if (setting.type != ExtensionSettingType::SLIDER && setting.type != ExtensionSettingType::TEXT && setting.type != ExtensionSettingType::TOGGLE)
            return nonstd::unexpected("Unknown setting type: '" + std::to_string(static_cast<int>(setting.type)) + "'");
